...
- **Sync the collection.** `social <platform> sync <collection>` walks it into the cache until the provider completes, reaches the stored checkpoint, or hits `--since`. Bare `social <platform> sync` lists the collections with their last-synced time. `--since <ISO|N:days>` bounds time-series collections after their first full sync. `social <platform> sync <collection> --reset` clears that local mirror and sync state without upstream calls; it rejects `--since`.
