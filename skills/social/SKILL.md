...
- Logged in but no connected row for the platform - run `social account connect linkedin` or `social account connect x` without `--account-id`. The server creates or resumes a partial `connected_accounts` row, purchases only that provider's plan or next seat, or the applicable legacy Social Pro seat during transition, and returns its `accountId`. Pass the exact ID as `--account-id <accountId>` on every retry: `pending_billing` has a `paymentURL` only when the bank requires customer action, `pending_approval` has the hosted `connectURL`, and `connected` has the completed account. A connected ID is stable when replayed; omit `--account-id` after completion only to connect another account.
